...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier verifyCaller (address _address) {
    require(msg.sender == _address, 'Can not verify caller');
    _;
  }

  modifier paidEnough(uint _price) {
    require(msg.value >= _price, 'Paid Enough');
    _;
  }
  ...
  modifier shipped(uint _sku) {
    require(items[_sku].state == State.Shipped, 'Item has not been shipped yet');
    _;
  }
  modifier received(uint _sku) {
    require(items[_sku].state == State.Received, 'Item has not been received yet');
    _;
  }
